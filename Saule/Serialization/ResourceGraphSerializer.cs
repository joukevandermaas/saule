...
{
    internal class ResourceGraphSerializer : IResourceSerializer
    {
        ...
        public ResourceGraphSerializer(
            object value,
            ApiResource type,
            Uri baseUrl,
        ...
        {
            var tokens = graph.DataNodes.Select(n => SerializeNode(n));
            if (_value is System.Collections.IEnumerable)
            {
                if (tokens.Count() == 0)
                {
                    return new JArray();
        ...
        {
            var tokens = graph.IncludedNodes.Select(n => SerializeNode(n));
            if (tokens.Count() == 0)
            {
                return null;
            }
        ...
        private JObject SerializeNode(ResourceGraphNode node)
        {
